...
export default function HeroSection({
  ...
  const imgSrc = proxiedMediaSrc(imageUrl)
    ...
    <SectionCard className="hero-section">
      <div className="grid items-center gap-8 md:grid-cols-[1.2fr,1fr]">
        <div>
          ...
          {subtitle ? (
            <p className="mt-4 max-w-lg text-[color:var(--text-secondary)]">{subtitle}</p>
          ) : null}
          <div className="mt-6 flex flex-wrap gap-3">
            <ActionButton href="#contact" className="px-5 py-2.5">
              Download CV
            </ActionButton>
            <ActionButton href="#projects" variant="secondary" className="px-5 py-2.5">
              View My Work
            </ActionButton>
          </div>
          ...
          <div className="relative">
            {imgSrc ? (
              <img
                src={imgSrc}
                alt={imageAlt || ''}
                className="h-64 w-64 rounded-full border border-[color:var(--border-strong)] object-cover md:h-72 md:w-72"
              />
            ) : (
